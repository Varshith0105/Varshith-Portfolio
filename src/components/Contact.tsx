  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('send-contact-email', {
        body: { name: formState.name, email: formState.email, message: formState.message },
      });
      if (error) throw error;
      playSound("confirm");
      toast.success("Message sent successfully!", { description: "I'll get back to you soon. Thank you for reaching out!" });
      setFormState({ name: "", email: "", message: "" });
    } catch (error: any) {
      console.error("Error sending message:", error);
      toast.error("Failed to send message", { description: "Please try again or contact me directly via email." });
    } finally {
  ...
  return (
    <section id="contact" className="section-padding relative overflow-hidden" ref={ref}>
      ...
      <div className="container-custom relative z-10">
        ...
        >
          <p className="text-primary font-medium mb-4 tracking-[0.3em] text-xs uppercase">Get In Touch</p>
        ...
        <div className="grid lg:grid-cols-2 gap-16 max-w-5xl mx-auto">
          ...
          >
            <div>
              <h3 className="font-display font-semibold text-2xl mb-6">Let's Connect</h3>
            ...
            <div className="space-y-5">
              ...
              >
                ...
                <div>
                  ...
                  <p className="font-medium group-hover:text-primary transition-colors duration-300">varshithjulakanti@gmail.com</p>
                </div>
              ...
              >
                ...
                <div>
                  ...
                  <p className="font-medium group-hover:text-primary transition-colors duration-300">+91 7842524222</p>
                </div>
                <ArrowUpRight size={18} className="ml-auto text-muted-foreground group-hover:text-primary transition-all duration-300 transform group-hover:translate-x-1 group-hover:-translate-y-1" />
              </motion.a>

              <motion.div className="flex items-center gap-5 p-6 glass-card rounded-2xl" whileHover={{ x: 6 }}>
          ...
          >
            <div className="space-y-7">
              <div>
                <label htmlFor="name" className="block text-sm font-medium mb-3 text-muted-foreground">Name</label>
                <input type="text" id="name" value={formState.name} onChange={(e) => setFormState({ ...formState, name: e.target.value })}
              ...
              <div>
                <label htmlFor="email" className="block text-sm font-medium mb-3 text-muted-foreground">Email</label>
                <input type="email" id="email" value={formState.email} onChange={(e) => setFormState({ ...formState, email: e.target.value })}
                  className="w-full px-5 py-4 rounded-xl bg-muted/50 border border-border/50 focus:border-primary/50 focus:outline-none focus:ring-2 focus:ring-primary/10 transition-all duration-300 text-foreground placeholder:text-muted-foreground/50"
                  placeholder="your@email.com" required />
              </div>
              <div>
                <label htmlFor="message" className="block text-sm font-medium mb-3 text-muted-foreground">Message</label>
                <textarea id="message" value={formState.message} onChange={(e) => setFormState({ ...formState, message: e.target.value })}
